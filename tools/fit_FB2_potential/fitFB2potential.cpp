...
int main (int argc, char* argv[])
{
	...
	if (argc < 10)
	{
		...
		cout << "fitFB2potential xy-file/U-file xBins yBins xNmax yNmax min_x_angle_DEG min_y_angle_DEG U-flag Temperature" << endl << endl;
		cout << "U-flag = 0 for xy-file, 1 for U (in kJ/mol) 2D matrix" << endl << endl;
		return 1;
	}
	...
	if (!havePotential) // Build U from trajectory
	{
		// 3. PARSE INPUT FILE
	...
	else // Read in U
	{
		...
		cout << "* Reading 2D histogram from file..." << endl;

		double conv = 1000.0 / (8.314 * temperature);
		...
		if (fabs(minU) > 1.0e-13) {for (int i = 0; i < xBins * yBins; i++) H.at(i) -= minU;}
